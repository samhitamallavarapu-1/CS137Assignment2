...
# ) -> nn.Module:
#     """
#     Build a torchvision pretrained model and replace its final classifier layer
#     with a new layer that matches the weather predictor output dimension.

#     Supported model families in this helper:
...
#     ----------
#     model_name : str
#         Name of torchvision model.
#     n_outputs : int
#         Number of outputs for the weather predictor.
#     pretrained : bool
#         Whether to initialize from torchvision pretrained weights.

#     Returns
#     -------
#     model : nn.Module
#         Model with final layer replaced.
#     """
...
#         weights_enum = _get_default_weights(model_name) if pretrained else None
...
#         if not isinstance(model.classifier, nn.Sequential) or len(model.classifier) < 2:
#             raise ValueError(f"Unexpected classifier structure for {model_name}")
...
#     """
#     Load a saved state_dict into an already-constructed model.

#     Parameters
#     ----------
#     model : nn.Module
#         Model architecture already created.
#     weights_path : str
#         Path to .pth file.
#     device : torch.device or None
#         Device to map weights to.
...
#     """
#     Return the torchvision default weights enum for a supported model.
#     """
#     weights_lookup = {
...
# # ----------------------------
# # Final layer / block helpers
# # ----------------------------

# def get_final_layer(model: nn.Module) -> nn.Module:
#     """
#     Return the final trainable prediction layer for supported architectures.
#     """
...
#     """
#     Return blocks ordered from output-side to input-side for gradual unfreezing.

#     For ResNet:
#       [fc, layer4, layer3, layer2, layer1, conv1/bn1]

#     For DenseNet / EfficientNet / MobileNet / ViT:
#       returns a reasonable coarse ordering.
#     """
...
#             model.features.denseblock4 if hasattr(model.features, "denseblock4") else model.features,
#             model.features.transition3 if hasattr(model.features, "transition3") else nn.Identity(),
#             model.features.denseblock3 if hasattr(model.features, "denseblock3") else nn.Identity(),
#             model.features.transition2 if hasattr(model.features, "transition2") else nn.Identity(),
#             model.features.denseblock2 if hasattr(model.features, "denseblock2") else nn.Identity(),
#             model.features.transition1 if hasattr(model.features, "transition1") else nn.Identity(),
#             model.features.denseblock1 if hasattr(model.features, "denseblock1") else nn.Identity(),
#             model.features.conv0 if hasattr(model.features, "conv0") else nn.Identity(),
#         ]
#         return [b for b in blocks if not isinstance(b, nn.Identity)]

#     # EfficientNet / MobileNet
#     if hasattr(model, "features") and hasattr(model, "classifier"):
#         features = list(model.features.children())
#         blocks = [get_final_layer(model)] + features[::-1]
#         return blocks

#     # ViT
#     if hasattr(model, "encoder") and hasattr(model, "heads"):
#         encoder_layers = list(model.encoder.layers.children())
#         blocks = [get_final_layer(model)] + encoder_layers[::-1]
#         return blocks

#     # Fallback: final layer only
...
#     """
#     Train only the final prediction layer.
#     """
...
# def set_train_mode_gradual_unfreeze(
#     model: nn.Module,
#     epoch: int,
#     unfreeze_every: int = 1,
# ) -> int:
#     """
#     Gradually unfreeze from output-side blocks to input-side blocks.

#     At epoch 1, only final layer is trainable.
#     Every `unfreeze_every` epochs, one additional earlier block is unfrozen.

#     Returns
#     -------
#     n_blocks_unfrozen : int
#         Number of output-side blocks currently trainable.
#     """
#     freeze_all_parameters(model)
...
# def get_optimizer_for_trainable_params(model: nn.Module, lr: float = 1e-3, weight_decay: float = 0.0):
#     """
#     Build optimizer using only parameters with requires_grad=True.
#     """
...
#     inputs shape for torchvision models:
#       (B, C, H, W)
#     targets shape:
#       (B, n_outputs)
#     """
...
#     for i, val in enumerate(rmse.tolist()):
#         metrics[f"rmse_{i}"] = float(val)
...
#     train_mode: str = "full",
#     unfreeze_every: int = 1,
#     output_model_path: str = "transfer_model_weights.pth",
#     early_stopping_patience: Optional[int] = None,
# ) -> List[Dict]:
#     """
#     Train a transfer-learning model with one of three modes:

#       - train_mode="full"
#       - train_mode="last_layer"
#       - train_mode="gradual_unfreeze"
...
#         Weight decay.
#     train_mode : str
#         One of {"full", "last_layer", "gradual_unfreeze"}.
#     unfreeze_every : int
#         For gradual unfreezing, add one earlier block every this many epochs.
...
#         val_metrics = evaluate_transfer_model(
#             model=model,
...
#     # Example:
#     # model = get_pretrained_backbone(model_name="resnet50", n_outputs=6, pretrained=True)
#     # model = load_model_weights(model, "transfer_model_weights.pth")
#     #
#     # Then train with one of:
#     #   train_mode="full"
#     #   train_mode="last_layer"
#     #   train_mode="gradual_unfreeze"
#     #
...
#     #     train_mode="gradual_unfreeze",
#     #     unfreeze_every=2,
#     #     output_model_path="resnet50_weather_best.pth",
