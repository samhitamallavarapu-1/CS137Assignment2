...
#         args.output_dir / "normalization_stats.pt",
#     )
#     print(f"Saved normalization stats to {args.output_dir / 'normalization_stats.pt'}")

#     device = torch.device(args.device)

#     # Important change: training model expects already-normalized inputs
#     model = get_model(metadata, for_evaluation=False).to(device)
#     model.train()
...
#         for start in tqdm(range(0, len(shuffled), args.batch_size), desc=f"Epoch {epoch}"):
...
#             torch.save(model.state_dict(), args.output_dir / "my_cnn_weights.pth")
#             print(f"  Saved best model to {args.output_dir / 'my_cnn_weights.pth'}")

#         with open(args.output_dir / "training_metrics.json", "w") as f:
...
def main():
    ...
    best_model_path = args.output_dir / "my_cnn_weights.pth"
